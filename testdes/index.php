...
function addField($rub, $name, $error)
{
	...
	echo '<tr><td class="col1"><label for="' . $name . '">' . ucfirst($name) . ' :</label></td>';
	...
	echo '></td><td class="col3"><span class="lerreur lerreurstyle1">' . $error . '</span></td></tr>';
}

function addFieldTwo($one, $nb, $for = true)
{	
	$to_echo = '<table class="table_type3">
			<caption>'.($type=($for)?'Formation':'Experience').'<!-- n°'.$nb.'--></caption>
			<tr>
			<td class="col1"><mark onclick="suppression(this, \''.($type=($for)?'for':'exp').'\')">X</mark></td>
			<td class="td_stop">
				<table>
				<tr>
					<td class="sscol1">Du :</td>
					<td class="sscol2"><input name="" id="'.($type=($for)?'for':'exp').'_datedebut_'.$nb.'" type="date" class="normal_input" value="'.$one['datedebut'].'"></td>
					<td class="sscol3">Au :</td>
					<td class="sscol4"><input name="" id="'.($type=($for)?'for':'exp').'_datefin_'.$nb.'" type="date" class="normal_input" value="'.$one['datefin'].'"></td>
				</tr>
				<tr>
					<td class="sscol1">'.($nomou=($for)?'Intitulé':'Poste occupé').' :</td><td class="sscol2"><input name="" id="'.($type=($for)?'for':'exp').'_prquoi_'.$nb.'" type="text" class="normal_input" value="'.$one['prquoi'].'"></td>
					<td class="sscol3">'.($nomou=($for)?'Organisme':'Entreprise').' :</td><td class="sscol4"><input name="" id="'.($type=($for)?'for':'exp').'_nomou_'.$nb.'" type="text" class="normal_input" value="'.$one['nomou'].'"></td>
				</tr>
				<tr>
					<td class="sscol1">Ville :</td><td class="ssco2"><input name="" id="'.($type=($for)?'for':'exp').'_ville_'.$nb.'" type="text" class="normal_input" value="'.$one['ville'].'"></td>
					<td class="ssco3">Département :</td>
					<td class="ssco4"><input name="" id="'.($type=($for)?'for':'exp').'_codedep_'.$nb.'" type="text" class="normal_input" value="'.$one['codedep'].'"></td>
				</tr>
				</table>
			</td>
			<td class="col3">
...
function addFieldThree($nb, $one)
{
	echo '<div class="one_item"><table class="table_type3"><tr>
		<td class="col1"><mark onclick="suppression(this, \'inf\')">X</mark></td>
		<td class="td_stop">
			<table><tr><td class="td01-t3">Libellé :<br /></td>
				<td class="td02-t3"><input type="text" name="" id="inf_libelle_' . $nb . '" class="normal_input" placeholder="Loisirs, Langues..." value="' . $one['libelle'] . '" /></td>
				
			</tr><tr>
				<td class="td01-t3">Contenu :</td>
				<td class="td02-t3"><input type="text" name="" id="inf_contenu_' . $nb . '" class="normal_input" value="' . $one['contenu'] . '" /></td>
		
			</tr></table>
		</td>
		<td class="col3"><span class="lerreur lerreurstyle1">Les données saisies ne peuvent être traitées...</span></td>
		</tr></table></div>';
}
?>
	...
	<body>
		...
		<div class="rubrique" id="ide">
			...
			<table class="table_type2">
				...
				?>
				<tr>
					<td class="col1"><label for="permis">Permis de conduire :</label></td>
					<td class="td_stop"><select name="ide_permis" id="ide_permis"><option selected="selected">Pas de permis</option><option>Permis B</option><option>Permis D</option></select></td>
					<td class="col3"><span class="lerreur lerreurstyle1">Champ à renseigner...</span></td>
				</tr>
				<tr>
					<td class="col1"><label for="vehicule">Véhicule :</label></td>
					<td class="td_stop"><select name="ide_vehicule" id="ide_vehicule"><option selected="selected">Non</option><option>Oui</option><option>Moto / Scooter</option></select></td>
					<td class="col3"><span class="lerreur lerreurstyle1">Champ à renseigner...</span></td>
				</tr>
			</table>
		</div>
